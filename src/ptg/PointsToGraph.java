...
//		if(objSet.isEmpty()) throw new IllegalArgumentException("[AssimilateObjects] Set empty for "+l.toString()+"."+f.toString());
	...
	public void cascadeCV(Local l, ConditionalValue cv, Map<ObjectNode, EscapeStatus> summary, SootMethod m) {
		//System.out.println(" In cacade CV method is : "+ m);
		if (!vars.containsKey(l)) return;
		HashSet<ObjectNode> visited = new HashSet<ObjectNode>();
		for (ObjectNode o : vars.get(l)) {
			HashSet<ObjectNode> visitedfield = new HashSet<ObjectNode>();
			if (summary.containsKey(o)) {
				summary.get(o).addEscapeState(cv);
			} else {
				summary.put(o, new EscapeStatus(cv));
			}
			recursiveCascadeES(o, cv, summary, visited, m, visitedfield);
			visited.add(o);
			//System.out.println("Visted : "+ visited);
		}
	}

	public void recursiveCascadeES(ObjectNode parent, ConditionalValue cv,  Map<ObjectNode, EscapeStatus> summary, HashSet<ObjectNode> visited, SootMethod m, HashSet<ObjectNode> visitedfield) {
		if (visited.contains(parent)) return;
		if (!fields.containsKey(parent)) return;
		if (visitedfield.contains(parent)) return;
		//HashSet<ObjectNode> children = new HashSet<ObjectNode>();
/* 		debug code
*		System.out.println("-----------------");
*		System.out.println("1. Parent is: "+ parent.toString());
*		System.out.println("2. CV is: "+ cv.toString());
*		System.out.println("3. Summary as of now: "+ summary.toString());
*		System.out.println("4. Done map value: "+ done.toString());
*		System.out.println("-----------------");
*/
		visitedfield.add(parent);
		Set<ObjectNode> objtemp = new HashSet<>();
		for(SootField f : fields.get(parent).keySet()) {
			ConditionalValue cv_with_field = cv.addField(f);
			for (ObjectNode obj : fields.get(parent).get(f)) {
				if(visitedfield.contains(obj)) {
					return;
				}
				for (EscapeState c : summary.get(parent).status) {
					summary.get(obj).addEscapeState(c);
				}
				//System.out.println("CV2 : "+ cv_with_field);
				summary.get(obj).addEscapeState(cv_with_field);
				objtemp.add(obj);
				if(fields.containsKey(obj) && !visited.contains(obj))
					recursiveCascadeES(obj, cv_with_field, summary, visited, m, visitedfield);
			}
		}

//
//		for (Map.Entry<SootField, Set<ObjectNode>> entry : fields.get(parent).entrySet()) {
//			//System.out.println("Field is for object: "+ parent + " is : "+ fields.get(parent));
//			ConditionalValue cv2 = cv.addField(entry.getKey());
//			//System.out.println("CV2 at first : "+ cv2.toString());
//			for (ObjectNode object : entry.getValue()) {
////				if(m.getBytecodeSignature().equals("<sun.net.www.protocol.http.HttpURLConnection: <clinit>()V>")) {
////					System.out.println("Reached inside recursive-cascadeES with current object "+ object );
////					System.out.println("Adding new CV : "+ cv2);
////				}
...
//
//				for (EscapeState c : summary.get(parent).status) {
//					summary.get(object).addEscapeState(c);
//				}
//				//summary.get(object).addEscapeState(cv);
//				summary.get(object).addEscapeState(cv2);
//				//System.out.println("CV2 at second : "+ cv2.toString());
//				recursiveCascadeES(object, cv2, summary, done, m);
//			}
//			//ConditionalValue cv2 = cv.addField(entry.getKey());
//			//entry.getValue().forEach(object -> summary.get(object).addEscapeState(cv2));
...
//		}
	...
	public void recursiveCascadeEscape(ObjectNode object, Map<ObjectNode, EscapeStatus> summary, HashSet<ObjectNode> done) {
		...
		//if (!fields.containsKey(object)) return;
