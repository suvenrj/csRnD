...
public class StoreStmt {
	public static void handle(Unit u, PointsToGraph ptg, Map<ObjectNode, EscapeStatus> summary, SootMethod m) {
		...
		if (lhs instanceof JInstanceFieldRef) {
			...
			if (rhs instanceof StringConstant || rhs instanceof NullConstant || rhs instanceof ClassConstant) {
				...
				StaticStoreStmt(u, ptg, summary, m);
	...
	private static void StaticStoreStmt(Unit u, PointsToGraph ptg, Map<ObjectNode, EscapeStatus> summary, SootMethod m) {
		//ptg.cascadeEscape((Local) ((JAssignStmt) u).getRightOp(), summary);
		Local rhs = (Local) ((JAssignStmt) u).getRightOp();
		Set<ObjectNode> rhsObjSet = ptg.vars.get(rhs);
		ConditionalValue cv = new ConditionalValue(m, new ObjectNode(0, ObjectType.global), true, 0);
		ptg.cascadeCV(rhs, cv, summary, m);
//		for (ObjectNode o : rhsObjSet) {
