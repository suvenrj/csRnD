...
// 		}
		...
		for(Edge edge: edges) {
			...
			// If method is not already processed, first analyse it
			StaticAnalyser.shouldWeProcessMethod(method);
            ...
            for (int i = 0; i < paramCount; i++) {
                ...
                for (Map.Entry<SootField, Set<ObjectNode>> entry : pointingTo.entrySet()) {
                    for (ObjectNode fieldObj : entry.getValue()) {
						...
						|| (paramMapping.containsKey(obj) && paramMapping.get(obj) == null)
						|| (paramMapping.containsKey(fieldObj) && paramMapping.get(fieldObj) == null)) {
                            // If paramsMapping does not have the object, it can happen if null is passed
							// It can also happen that paramMapping object is null
                            continue;
                        }
