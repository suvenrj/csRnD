...
public class StaticAnalyser extends BodyTransformer {
	...
	public static void shouldWeProcessMethod(SootMethod methodCaller) {
        ...
        if (methodsProcessing.contains(methodCaller)) {
            // Recursive loop
            // ASK - See what to do
			// Currently we ignore and return successfully
            return;
        }

		methodsProcessing.add(methodCaller);

		// We process this method
		if (methodCaller.hasActiveBody()) {
			processBody(methodCaller.getActiveBody());
		}
